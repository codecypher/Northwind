...
-- SET ANSI_NULLS settings are captured and used for subsequent invocations
-- of that stored procedure.

-- Strings inside double quotation marks are now treated as object names
-- and string literals must be enclosed in single quotes.
SET QUOTED_IDENTIFIER ON
GO

-- Must be ON when you are creating or changing indexes on computed columns
-- or indexed views.
-- A SELECT statement that uses WHERE column_name = NULL (<> NULL)
-- returns zero rows even if there are null (nonnull) values in column_name.
SET ANSI_NULLS ON
GO
...
-- PRINT 'ErrorId = ' + CAST(@ErrorId AS VARCHAR(36))
...
-- EXEC ELMAH_GetErrorsXml 'Application',0,15,@total OUTPUT
...
-- PRINT 'PageIndex = ' + CAST(@PageIndex AS CHAR(10))
-- PRINT 'PageSize = ' + CAST(@PageSize AS CHAR(10))
